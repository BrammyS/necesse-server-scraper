...
public class Repository<T> : IRepository<T> where T : BaseDocument
{
    ...
    {
        var result = await _mongoCollection.AsQueryable().Sample(1).FirstOrDefaultAsync().ConfigureAwait(false);
        return result;
    }
    ...
    {
        var result = await _mongoCollection.AsQueryable().Sample(amount).ToListAsync().ConfigureAwait(false);
        return result;
    }
}